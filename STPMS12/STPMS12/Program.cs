...
namespace Laba12
{
    ...
    public static class Reflection
    {
        ...
        {
            string path = @"D:\БГТУ\2 курс\ООП\STPMS12\message.txt";
            ...
            FileWriter(info, "Специальные Методы:");
            Type type = t.GetType();
            MethodInfo[] Mi = type.GetMethods();
            foreach (MethodInfo mi in Mi)
            {
                FileWriter(info, $"    [{mi.ToString()}]");
                Console.WriteLine("найден метод");
            }
        ...
        {
            FileStream f = new FileStream(@"D:\БГТУ\2 курс\ООП\STPMS12\message.txt", FileMode.Open);
